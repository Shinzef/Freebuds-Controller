...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "JNI_BRIDGE", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "JNI_BRIDGE", __VA_ARGS__)

// Helper to get the C++ Device object from a Java long
static Device* get_device(jlong ptr) {
    return reinterpret_cast<Device*>(ptr);
}
...
Java_com_example_freebuds_1flutter_MainActivity_nativeInit(JNIEnv *env, jobject thiz, jobject bt_manager) {
    ...
    context->findDeviceMethodId = env->GetMethodID(managerClass, "findDeviceByName", "(Ljava/lang/String;)Landroid/bluetooth/BluetoothDevice;");
    context->connectMethodId = env->GetMethodID(managerClass, "connect", "(Ljava/lang/String;)Z");
    context->isConnectedMethodId = env->GetMethodID(managerClass, "isConnected", "()Z");

    env->DeleteLocalRef(managerClass);

    if (!context->findDeviceMethodId || !context->connectMethodId || !context->isConnectedMethodId) {
...
Java_com_example_freebuds_1flutter_MainActivity_nativeTestConnection(JNIEnv *env, jobject thiz, jlong instance_ptr, jstring device_name) {
    ...
    jboolean success = env->CallBooleanMethod(context->bluetoothManagerInstance, context->connectMethodId, jname);
...
// Updated function name for Flutter package
extern "C" JNIEXPORT jlong JNICALL
Java_com_example_freebuds_1flutter_MainActivity_createDevice(JNIEnv *env, jobject thiz, jobject bt_manager) {
    ...
    try {
        ...
        auto bt_client = std::make_unique<BluetoothSppClientAndroid>(vm, bt_manager);
...
Java_com_example_freebuds_1flutter_MainActivity_freeDevice(JNIEnv *env, jobject thiz, jlong device_ptr) {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeConnect(JNIEnv *env, jobject thiz, jlong device_ptr, jstring address) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeDisconnect(JNIEnv *env, jobject thiz, jlong device_ptr) {
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeIsConnected(JNIEnv *env, jobject thiz, jlong device_ptr) {
    if (device_ptr == 0) return false;
...
static GestureAction intToGestureAction(jint action) {
    switch (action) {
        ...
        case 11: return GestureAction::ANSWER_CALL;
...
Java_com_example_freebuds_1flutter_MainActivity_nativeGetGestureSettings(JNIEnv *env, jobject thiz, jlong device_ptr) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return nullptr;
    ...
    // Populate the map
    add_int_to_map("double_tap_left", gestureActionToInt(settings.double_tap_left));
    add_int_to_map("double_tap_right", gestureActionToInt(settings.double_tap_right));
    add_int_to_map("triple_tap_left", gestureActionToInt(settings.triple_tap_left));
    add_int_to_map("triple_tap_right", gestureActionToInt(settings.triple_tap_right));
    add_int_to_map("long_tap_left", gestureActionToInt(settings.long_tap_left));
    add_int_to_map("long_tap_right", gestureActionToInt(settings.long_tap_right));
...
// Updated function name for Flutter package
extern "C" JNIEXPORT void JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeCleanup(JNIEnv *env, jobject thiz, jlong instance_ptr) {
...
extern "C" JNIEXPORT jobject JNICALL
Java_com_example_freebuds_1flutter_MainActivity_getBatteryFromNative(JNIEnv *env, jobject thiz, jlong device_ptr) {
    ...
    try {
        ...
        jobject leftValue = env->NewObject(integerClass, integerConstructor, battery_info->left);
        jobject rightValue = env->NewObject(integerClass, integerConstructor, battery_info->right);
        jobject caseValue = env->NewObject(integerClass, integerConstructor, battery_info->case_level);
        // --- MODIFIED BLOCK END ---
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_setAncModeNative(JNIEnv *env, jobject thiz, jlong device_ptr, jint mode) {
...
extern "C" JNIEXPORT jobject JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeGetEqualizerInfo(JNIEnv *env, jobject thiz, jlong device_ptr) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return nullptr;
    ...
    jobject currentIdValue = env->NewObject(intClass, intCtor, eq_info.current_preset_id);
    ...
    jintArray builtInIdsArray = env->NewIntArray(eq_info.built_in_preset_ids.size());
    // Note: jint is not always the same as uint8_t, but for small numbers it's fine.
    // A proper conversion would be needed if values could be large.
    std::vector<jint> temp_built_in(eq_info.built_in_preset_ids.begin(), eq_info.built_in_preset_ids.end());
    env->SetIntArrayRegion(builtInIdsArray, 0, temp_built_in.size(), temp_built_in.data());
    ...
    env->SetIntArrayRegion(valuesArray, 0, temp_values.size(), temp_values.data());
    ...
    env->CallObjectMethod(returnMap, putMethod, customPresetsKey, customPresetsList);
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeSetEqualizerPreset(JNIEnv *env, jobject thiz, jlong device_ptr, jint preset_id) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
    LOGI("nativeSetEqualizerPreset called with id: %d", preset_id);
    return get_device(device_ptr)->set_equalizer_preset(static_cast<uint8_t>(preset_id));
}

// --- END OF EQUALIZER CONTROL ---
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeGetWearDetectionStatus(JNIEnv *env, jobject thiz, jlong device_ptr) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeSetWearDetection(JNIEnv *env, jobject thiz, jlong device_ptr, jboolean enable) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeGetLowLatencyStatus(JNIEnv *env, jobject thiz, jlong device_ptr) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeSetLowLatency(JNIEnv *env, jobject thiz, jlong device_ptr, jboolean enable) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
...
Java_com_example_freebuds_1flutter_MainActivity_nativeGetSoundQuality(JNIEnv *env, jobject thiz, jlong device_ptr) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return 0; // Default to "Connection"
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeSetSoundQuality(JNIEnv *env, jobject thiz, jlong device_ptr, jint preference) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
...
extern "C" JNIEXPORT jobject JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeGetAncStatus(JNIEnv *env, jobject thiz, jlong device_ptr) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return nullptr;
    ...
    LOGI("Raw ANC status - mode: %d, level: %d", static_cast<int>(status.mode), static_cast<int>(status.level));
    ...
    int flutter_mode = 0;
    switch (status.mode) {
        case AncMode::NORMAL: flutter_mode = 0; break;
        case AncMode::CANCELLATION: flutter_mode = 1; break;
        case AncMode::AWARENESS: flutter_mode = 2; break;
        default: flutter_mode = 0; break;
    }
    ...
    if (status.mode == AncMode::AWARENESS) {
        ...
        flutter_level = is_voice_boost ? 1 : 0; // 1 = voice boost on, 0 = voice boost off
    } else if (status.mode == AncMode::CANCELLATION) {
        switch (status.level) {
            case AncLevel::COMFORTABLE: flutter_level = 0; break;
            case AncLevel::NORMAL_CANCELLATION: flutter_level = 1; break;
            case AncLevel::ULTRA: flutter_level = 2; break;
            case AncLevel::DYNAMIC: flutter_level = 3; break;
            default: flutter_level = 0; break;
        }
    }

    LOGI("Converted to Flutter - mode: %d, level: %d", flutter_mode, flutter_level);
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeSetAncLevel(JNIEnv *env, jobject thiz, jlong device_ptr, jint level) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
    LOGI("nativeSetAncLevel called with: %d", level);

    AncLevel level_enum;
    switch (level) {
        case 0: level_enum = AncLevel::COMFORTABLE; break;
        case 1: level_enum = AncLevel::NORMAL_CANCELLATION; break;
        case 2: level_enum = AncLevel::ULTRA; break;
        case 3: level_enum = AncLevel::DYNAMIC; break;
        case 4: level_enum = AncLevel::VOICE_BOOST; break;          // Voice Boost ON
        case 6: level_enum = AncLevel::NORMAL_AWARENESS; break;     // Normal Awareness (Voice Boost OFF)
        default:
...
extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeSetDoubleTapAction(JNIEnv *env, jobject thiz, jlong device_ptr, jint side, jint action) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
    LOGI("nativeSetDoubleTapAction called with side: %d, action: %d", side, action);
    return get_device(device_ptr)->set_double_tap_action(intToEarSide(side), intToGestureAction(action));
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeSetTripleTapAction(JNIEnv *env, jobject thiz, jlong device_ptr, jint side, jint action) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
    LOGI("nativeSetTripleTapAction called with side: %d, action: %d", side, action);
    return get_device(device_ptr)->set_triple_tap_action(intToEarSide(side), intToGestureAction(action));
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeSetLongTapAction(JNIEnv *env, jobject thiz, jlong device_ptr, jint side, jint action) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
    LOGI("nativeSetLongTapAction called with side: %d, action: %d", side, action);
    return get_device(device_ptr)->set_long_tap_action(intToEarSide(side), intToGestureAction(action));
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_example_freebuds_1flutter_MainActivity_nativeSetSwipeAction(JNIEnv *env, jobject thiz, jlong device_ptr, jint action) {
    if (device_ptr == 0 || !get_device(device_ptr)->is_connected()) return false;
...
Java_com_example_freebuds_1flutter_MainActivity_nativeCreateOrUpdateCustomEqualizer(
    ...
    LOGI("Calling create_or_update_custom_equalizer with id: %d, name: %s", id, preset.name.c_str());
