...
bool Device::set_anc_level(AncLevel level) { return m_writer ? m_writer->set_anc_level(level) : false; }
bool Device::set_wear_detection(bool e) { return m_writer ? m_writer->set_wear_detection(e) : false; }
bool Device::set_low_latency(bool e) { return m_writer ? m_writer->set_low_latency(e) : false; }
bool Device::set_sound_quality_preference(SoundQualityPreference p) { return m_writer ? m_writer->set_sound_quality_preference(p == SoundQualityPreference::PRIORITIZE_QUALITY) : false; }
bool Device::set_double_tap_action(EarSide s, GestureAction a) { return m_writer ? m_writer->set_double_tap_action(s, a) : false; }
bool Device::set_triple_tap_action(EarSide s, GestureAction a) { return m_writer ? m_writer->set_triple_tap_action(s, a) : false; }
bool Device::set_swipe_action(GestureAction a) { return m_writer ? m_writer->set_swipe_action(a) : false; }
bool Device::set_long_tap_action(EarSide s, GestureAction a) { return m_writer ? m_writer->set_long_tap_action(s, a) : false; }
bool Device::set_long_tap_anc_cycle(EarSide s, AncCycleMode m) { return m_writer ? m_writer->set_long_tap_anc_cycle(s, m) : false; }
...
bool Device::dual_connect_action(const std::string& mac, uint8_t code) { return m_writer ? m_writer->dual_connect_action(mac, code) : false; }

// --- Read API (Complete) ---
std::optional<DeviceInfo> Device::get_device_info() {
    auto request = HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_DEVICE_INFO_READ, {7, 9, 10, 15, 24});
...
std::optional<BatteryInfo> Device::get_battery_info() {
    auto request = HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_BATTERY_READ, {1, 2, 3});
...
std::optional<GestureSettings> Device::get_all_gesture_settings() {
    ...
    if (auto r = send_and_get_response(HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_DUAL_TAP_READ, {1, 2, 4}), HuaweiCommands::CMD_DUAL_TAP_READ)) populate_gesture_settings(settings, *r);
    if (auto r = send_and_get_response(HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_TRIPLE_TAP_READ, {1, 2}), HuaweiCommands::CMD_TRIPLE_TAP_READ)) populate_gesture_settings(settings, *r);
    if (auto r = send_and_get_response(HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_LONG_TAP_SPLIT_READ_BASE, {1, 2}), HuaweiCommands::CMD_LONG_TAP_SPLIT_READ_BASE)) populate_gesture_settings(settings, *r);
    if (auto r = send_and_get_response(HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_LONG_TAP_SPLIT_READ_ANC, {1, 2}), HuaweiCommands::CMD_LONG_TAP_SPLIT_READ_ANC)) populate_gesture_settings(settings, *r);
    if (auto r = send_and_get_response(HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_SWIPE_READ, {1}), HuaweiCommands::CMD_SWIPE_READ)) populate_gesture_settings(settings, *r);
    return settings;
}

std::vector<DualConnectDevice> Device::get_dual_connect_devices() {
    std::vector<DualConnectDevice> devices;
    auto request = HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_DUAL_CONNECT_ENUMERATE, {1});
    ...
    if (m_client->send(request.to_bytes())) {
        for (const auto& bytes : m_client->receive_all()) {
            if (auto packet = HuaweiSppPacket::from_bytes(bytes)) {
                if (packet->command_id == bytes_to_u16(HuaweiCommands::CMD_DUAL_CONNECT_ENUMERATE[0], HuaweiCommands::CMD_DUAL_CONNECT_ENUMERATE[1])) {
...
std::optional<EqualizerInfo> Device::get_equalizer_info() {
    auto request = HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_EQUALIZER_READ, {2, 3, 8});
...
std::optional<SoundQualityPreference> Device::get_sound_quality_preference() {
    auto request = HuaweiSppPacket::create_read_request(HuaweiCommands::CMD_SOUND_QUALITY_READ, {1});
     if (auto response = send_and_get_response(request, HuaweiCommands::CMD_SOUND_QUALITY_READ)) {
        if (auto p = response->get_param(2); p && !p->empty()) {
            return (*p)[0] == 1 ? SoundQualityPreference::PRIORITIZE_QUALITY : SoundQualityPreference::PRIORITIZE_CONNECTION;
        }
    }
    return std::nullopt;
}
...
std::optional<HuaweiSppPacket> Device::send_and_get_response(const HuaweiSppPacket& request, const std::array<uint8_t, 2>& expected_response_cmd) {
    if (m_client->send(request.to_bytes())) {
        ...
        for (const auto& bytes : responses_bytes) {
            if (auto packet = HuaweiSppPacket::from_bytes(bytes)) {
                if (packet->command_id == bytes_to_u16(expected_response_cmd[0], expected_response_cmd[1])) {
                    return packet;
                }
            }
        }
...
void Device::populate_gesture_settings(GestureSettings& settings, const HuaweiSppPacket& packet) {
    if (packet.command_id == bytes_to_u16(HuaweiCommands::CMD_DUAL_TAP_READ[0], HuaweiCommands::CMD_DUAL_TAP_READ[1])) {
        if(auto p = packet.get_param(1)) settings.double_tap_left = int_to_gesture_action((*p)[0]);
        if(auto p = packet.get_param(2)) settings.double_tap_right = int_to_gesture_action((*p)[0]);
        if(auto p = packet.get_param(4)) settings.double_tap_incall = int_to_gesture_action((*p)[0]);
    } else if (packet.command_id == bytes_to_u16(HuaweiCommands::CMD_TRIPLE_TAP_READ[0], HuaweiCommands::CMD_TRIPLE_TAP_READ[1])) {
        if(auto p = packet.get_param(1)) settings.triple_tap_left = int_to_gesture_action((*p)[0]);
        if(auto p = packet.get_param(2)) settings.triple_tap_right = int_to_gesture_action((*p)[0]);
    } else if (packet.command_id == bytes_to_u16(HuaweiCommands::CMD_LONG_TAP_SPLIT_READ_BASE[0], HuaweiCommands::CMD_LONG_TAP_SPLIT_READ_BASE[1])) {
    ...
    } else if (packet.command_id == bytes_to_u16(HuaweiCommands::CMD_LONG_TAP_SPLIT_READ_ANC[0], HuaweiCommands::CMD_LONG_TAP_SPLIT_READ_ANC[1])) {
        if(auto p = packet.get_param(1)) settings.long_tap_anc_cycle_left = int_to_anc_cycle_mode((*p)[0]);
        if(auto p = packet.get_param(2)) settings.long_tap_anc_cycle_right = int_to_anc_cycle_mode((*p)[0]);
    } else if (packet.command_id == bytes_to_u16(HuaweiCommands::CMD_SWIPE_READ[0], HuaweiCommands::CMD_SWIPE_READ[1])) {
        if(auto p = packet.get_param(1)) settings.swipe_action = ((*p)[0] == 0) ? GestureAction::CHANGE_VOLUME : GestureAction::OFF;
    }
}
...
void Device::populate_equalizer_info(EqualizerInfo& info, const HuaweiSppPacket& packet) {
    if (packet.command_id != bytes_to_u16(HuaweiCommands::CMD_EQUALIZER_READ[0], HuaweiCommands::CMD_EQUALIZER_READ[1])) return;
...
AncStatus Device::parse_anc_status(const HuaweiSppPacket& packet) {
    ...
    if (auto p = packet.get_param(1); p && p->size() == 2) {
        ...
        // Add debugging
        std::cout << "Raw packet values - level_code: " << (int)level_code << ", mode_code: " << (int)mode_code << std::endl;

        switch (mode_code) {
            case 0: status.mode = AncMode::NORMAL; break;
            case 1: status.mode = AncMode::CANCELLATION; break;
            case 2: status.mode = AncMode::AWARENESS; break;
            default: status.mode = AncMode::UNKNOWN; break;
        }

        status.level = int_to_anc_level(mode_code, level_code);

        std::cout << "Parsed AncStatus - mode: " << (int)status.mode << ", level: " << (int)status.level << std::endl;
    }
    return status;
}